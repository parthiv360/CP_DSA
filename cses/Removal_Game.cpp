...
ll calc(ll l, ll r, vector<ll>&v, vector<vector<ll>>&dp){
    if(l>r)
    return 0;
    if(dp[l][r]!=-1)
    return dp[l][r];

    ll t= v[l]+min(calc(l+1,r-1,v,dp),calc(l+2,r,v,dp));
    ll nt= v[r]+min(calc(l+1,r-1,v,dp),calc(l,r-2,v,dp));

    return dp[l][r]=max(t,nt);
}
void solve()
